...
impl Table {
    ...
    pub fn from_dataframe(df: DataFrame, options: FromCsvOptions) -> Result<Table> {
        #[derive(Clone, Debug)]
        enum ColumnTypeOrInferStats<'a> {
            ColumnType(TableColumnType),
            InferStats(InferStats<'a>),
        ...
        let mut n_rows = Some(df.shape().0);
        let n_cols = df.shape().1;
        ...
        let column_types: Vec<TableColumnType> = if needs_infer {
            ...
            for i in (0..n_rows.unwrap()).into_iter() {
                let idx = IdxCa::new("idx", &[i as u32]);
                let row = df.take(&idx).unwrap();

                for (index, infer_stats) in infer_stats.iter_mut() {
                    let value = row.select_at_idx(*index).unwrap().get(0);
                    infer_stats.update(value.to_string().replace("\"", "").as_str());
                }
        ...
        for i in (0..n_rows).into_iter() {
            let idx = IdxCa::new("idx", &[i as u32]);
            let row = df.take(&idx).unwrap();

            for (j, column) in table.columns.iter_mut().enumerate() {
                let val = row.select_at_idx(j).unwrap().get(0);
                ...
                match column {
                    ...
                    TableColumn::Number(column) => {
                        ...
                        let value = match fast_float::parse::<f32, &[u8]>(n) {
                            Ok(value) if value.is_finite() => value,
                            _ => std::f32::NAN,
                        };
                        column.data.push(value);
                    ...
                    TableColumn::Enum(column) => {
                        let x = val.to_string().replace("\"", "");
                        let n = x.as_bytes();

                        let value = std::str::from_utf8(n)
                            .ok()
                            .and_then(|value| column.value_for_variant(value));
                        column.data.push(value);
                    }
                    TableColumn::Text(column) => {
                        ...
                        column.data.push(std::str::from_utf8(n)?.to_owned())
                    }
                }
            }
        }
